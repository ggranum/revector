...
import {AuthServiceCIF, UserAuthInfoIF, UserAuthTokenIF, EmailPasswordCredentials} from "./auth.service.interface";
...
export class FirebaseAuthService implements AuthServiceCIF {
  ...
  requestSignIn(payload: EmailPasswordCredentials): Observable<UserAuthInfoIF> {
    let loginCfg = {
      email: payload.email,
    ...
    let response = promise.then((fbAuthState: FirebaseAuthState) => {
      if (fbAuthState != null) {
        return this.userFromAuth(<UserAuthInfoIF>fbAuthState.auth)
      } else {
  ...
  requestSignUp(payload: EmailPasswordCredentials): Observable<UserAuthInfoIF> {
  ...
  populateNewAccountInfo(user: UserAuthInfoIF): Observable<any> {
    let p = <Promise<any>>this.angularFire.database.object(`/auth/${user.uid}`).set({
      uid: user.uid,
  ...
  globalEventObserver(): Observable<UserAuthTokenIF> {
    return this._auth.map((fbAuthState: FirebaseAuthState) => {
      ...
      if(fbAuthState && fbAuthState.auth){
        ...
        result.auth = this.userFromAuth(<UserAuthInfoIF>fbAuthState.auth)
      }
      return result
    })
  }

  private userFromAuth(fbAuth: UserAuthInfoIF): UserAuthInfoIF {
    return {
